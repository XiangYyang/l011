...
pub fn build(b: *std.Build) !void {
    ...
    // add startup-file
    elf.addAssemblyFile(b.path("boot/startup.s"));

    // add panic handler
    elf.addAssemblyFile(b.path("boot/panic.s"));

    // set ldscript
    const ldscript_file = "boot/link.x";
    elf.setLinkerScriptPath(b.path(ldscript_file));

    // chead module
    // This module has been generated by `chead.zig`
    // and it includes `@cDefine` to make use `@cInclude` easiler.
    // This module depends on all C source code.
    const chead = b.addExecutable(.{
        .name = "chead_ffi",
        .root_source_file = b.path("chead.zig"),
        .target = b.host,
    });

    const chead_step = b.addRunArtifact(chead);
    const output = chead_step.addOutputFileArg("chead.zig");

    // add c flags to arguments
    const arg_cflag_len = try std.fmt.allocPrint(alloc, "{}", .{c_flags.len});
    defer alloc.free(arg_cflag_len);
    chead_step.addArg(arg_cflag_len);
    chead_step.addArgs(c_flags);

    // create chead module
    const chead_mod = b.createModule(.{
        .root_source_file = output,
    });

    // cmake tool
    // to build all c source files, and generate compiled database for clangd
    var make = cmake.CMake.init(alloc, b, chead_mod);
    defer make.deinit();
    ...
    // add source file.
    try make.add_files(&.{
        .{ .source = .{ .base_dir = ".", .sub_dir = "boot", .flags = c_flags } },
        .{ .source = .{ .base_dir = driver_base, .sub_dir = "Src", .flags = c_flags } },
        .{ .source = .{ .base_dir = cubemx_out_base, .sub_dir = "Src", .flags = c_flags } },
    });

    // add chead module into elf.root_module
    elf.root_module.addImport("chead", chead_mod);
